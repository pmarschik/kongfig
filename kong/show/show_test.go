...
func TestRenderNoRenderer_Error(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("expected error when no renderer configured, got output:\n%s", buf.String())
	}
}
...
func TestRenderLayers(t *testing.T) {
	...
	f := &kongshow.Flags{LayersFlag: kongshow.LayersFlag{Layers: true}, FormatFlag: kongshow.FormatFlag{Format: "env"}}
	var buf bytes.Buffer
	if err := f.Render(context.Background(), &buf, k, plain.New(), kongfig.WithRenderNoComments()); err != nil {
		t.Fatal(err)
	}
