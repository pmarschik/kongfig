...
type FormatFlag struct {
	Format string `name:"format" config:"-" default:"" enum:"${kongrender_formats=,env,flags}" help:"Output format (${enum})."`
}
...
// When nil, rendering such layers returns an error.
type Flags struct {
...
func (f *Flags) Render(ctx context.Context, w io.Writer, k *kongfig.Kongfig, s kongfig.Styler, opts ...kongfig.RenderOption) error {
	...
	return k.RenderWith(ctx, w, &dataRenderer{format: format, s: s, renderers: renderers, defaultRenderer: f.DefaultRenderer}, allOpts...)
}
...
//
// Set DefaultRenderer to override the fallback renderer used when a layer
// has no native parser. When nil, rendering such layers returns an error.
...
func renderData(ctx context.Context, w io.Writer, format string, defaultRenderer kongfig.OutputProvider, data kongfig.ConfigData, s kongfig.Styler, renderers map[string]kongfig.OutputProvider) error {
	switch format {
	case "env":
...
// OutputProvider is used, falling back to "" (which requires DefaultRenderer or an explicit format).
...
func (r *dataRenderer) Render(ctx context.Context, w io.Writer, data kongfig.ConfigData) error {
	return renderData(ctx, w, r.format, r.defaultRenderer, data, r.s, r.renderers)
}
...
//  4. defaultRenderer (user-configured fallback via Flags.DefaultRenderer)
func renderLayerData(ctx context.Context, w io.Writer, layer kongfig.Layer, format string, defaultRenderer kongfig.OutputProvider, s kongfig.Styler, renderers map[string]kongfig.OutputProvider, data kongfig.ConfigData) error {
	...
	return fmt.Errorf("no renderer available for layer %q: register a parser or set DefaultRenderer on show.Flags", layer.Meta.Name)
}
...
// Always prepends "" (auto-detect) and appends "env", "flags" if not present.
