...
`Styler` is a plain Go interface. Embed `render.BaseStyler` to inherit pass-through
...
2. **Never format source annotations inline.** Always use:
   ...
   `SourceKind`/`SourceData`/`SourceKey`). Returns `""` when there is no source or
   when `render.NoComments(ctx)` is true — no separate check needed.
